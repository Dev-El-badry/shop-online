...
<div class="box box-primary">
  <div class="box-header with-border">
    <h3 class="box-title"><i class="fa fa-gear"></i> &nbsp;{{ trans('blog.update_blog_category') }}</h3>
  </div><!-- /.box-header -->
    ...
    @endif
  	...
  	<form action="{{ route('blogs.submit_action', $update_id) }}" method="POST">
  	{{ csrf_field() }}
    <div class="box-body">
	...
	<div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title" style="color: #f00">{{ trans('blog.update_blog_category') }}</h3>
...
<script>
	$('.delete_cat_assign').on('click', function() {
		if(confirm('{{ trans('cat_assign.alert_del') }}')){
			...
			var target_url = "{{ route('blogs.delete_cat_assign') }}";
			$.post(
				target_url,
				obj,
				function (data){
